          ...
          <div className="modal appointment-modal" role="dialog" aria-modal="true" aria-labelledby="appointment-title">
            ...
            <form className="appointment-form" onSubmit={addAppointment} noValidate>
              ...
              <section className="appointment-form-section">
                ...
                <details className="appointment-section-manager"><summary><span>Manage calendar sections</span><small>Add, reorder or delete</small></summary><div className="calendar-section-manager-body"><div className="inline-section-maker"><input aria-label="New calendar section" value={newAppointmentSection} onChange={(event) => setNewAppointmentSection(event.target.value)} placeholder="Name a new section" /><button type="button" onClick={() => addAppointmentSection()} disabled={!newAppointmentSection.trim()}>+ Add</button></div><div className="section-delete-list">{appointmentSections.map((item, index) => <div className="calendar-section-manage-row" key={item}><strong>{item}</strong><span className="calendar-section-row-actions"><button type="button" aria-label={`Move ${item} up`} disabled={index === 0} onClick={() => setAppointmentSections((current) => moveInList(current, index, -1))}><span aria-hidden="true">↑</span><small>Up</small></button><button type="button" aria-label={`Move ${item} down`} disabled={index === appointmentSections.length - 1} onClick={() => setAppointmentSections((current) => moveInList(current, index, 1))}><span aria-hidden="true">↓</span><small>Down</small></button><button type="button" className="calendar-section-delete" onClick={() => deleteAppointmentSection(item)}>{appointmentSectionDeleteConfirm === item ? `Confirm delete + plans` : "Delete"}</button></span></div>)}</div></div></details>
              </section>
              <section className="appointment-form-section">
              ...
              <section className={`appointment-form-section getting-ready-section ${appointmentAllDay ? "is-disabled" : ""}`} aria-disabled={appointmentAllDay}>
                <h3>Getting ready <span>optional</span></h3>
                {appointmentAllDay && <p className="all-day-disabled-note">Times, travel and reminders are unavailable for an all-day adventure.</p>}
                <span className="field-label">Reminder</span>
                <div className="frequency-picker reminder-picker">{[0, 5, 15, 30, 60].map((minutes) => <button key={minutes} type="button" disabled={appointmentAllDay} className={appointmentReminder === minutes ? "selected" : ""} onClick={() => setAppointmentReminder(minutes)}>{minutes ? `${minutes} min` : "Off"}</button>)}</div>
                <span className="field-label">Travel time</span><div className="frequency-picker reminder-picker">{[0, 10, 20, 30, 45, 60].map((minutes) => <button key={minutes} type="button" disabled={appointmentAllDay} className={appointmentTravel === minutes ? "selected" : ""} onClick={() => setAppointmentTravel(minutes)}>{minutes ? `${minutes} min` : "None"}</button>)}</div>{!appointmentAllDay && appointmentTravel > 0 && appointmentReminder > 0 && <p className="travel-note">Your notification will arrive {appointmentReminder} minutes before travel time.</p>}
                {!appointmentAllDay && appointmentReminder > 0 && !notificationsEnabled && <button type="button" className="notification-button" onClick={enableNotifications}>Enable browser reminders 🔔</button>}
              </section>
